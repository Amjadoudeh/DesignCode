...
//  DesignCode
//
//  Created by Amjad Oudeh on 19.04.22.
//
...
/// Creating the same shap using Canvas
        Canvas { context, size in
            context.fill(path(in: CGRect(x: 0, y: 0,width: size.width , height: size.height )), with: .linearGradient(Gradient(colors: [.pink, .blue]), startPoint: CGPoint(x: 0, y: 0), endPoint: CGPoint(x: 300, y: 310)))
        }
